...
| Skill                                        | Auslöser                                                                                              |
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `impeccable`                                 | Bei jeder UI-Arbeit (neue Komponente, Seite, Layout-Entscheidung). Kontext liegt in `.impeccable.md`. |
| `superpowers:brainstorming`                  | Vor jeder neuen Komponente oder Feature-Implementierung.                                              |
| `superpowers:verification-before-completion` | Vor "fertig"-Meldungen — sicherstellen, dass das Ergebnis dem Ziel entspricht.                        |

### Situativ

| Skill                             | Auslöser                                                                             |
| --------------------------------- | ------------------------------------------------------------------------------------ |
| `shape`                           | Beim Planen neuer Seiten oder Sektionen — UX-Discovery vor dem Code.                 |
| `superpowers:writing-plans`       | Vor größeren Implementierungen (Sanity-Integration, Kontaktformular, etc.).          |
| `frontend-design:frontend-design` | Wenn eine Design-Entscheidung unklar ist und `.impeccable.md` keine Antwort liefert. |

### Nicht verwenden

- `ui-ux-pro-max` — auf Tailwind/shadcn ausgelegt, passt nicht zum Custom-Properties-Ansatz
- `animate`, `colorize`, `glassmorphism` — widersprechen den Anti-Patterns in `.impeccable.md`
